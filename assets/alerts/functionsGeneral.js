...
function saveFactura(){
  ...
  var factura={
    ...
    razonFac: $('#razon_fac').val(),
    impFac: $('#imp_fac').val(),    
    autFac: setValCeroToStringNull($('#aut_fac').val()),
    conFac: setValCeroToStringNull($('#con_fac').val()),
    exeFac: setValCeroToStringNull($('#exe_fac').val()),
    iceFac: setValCeroToStringNull($('#ice_fac').val()),
...
$(document).ready(function() {
  $("input").focus(function() {
    $("#msg_error"+$(this).attr("id")).remove();
  });
  $(".error-fac").click(function() {
    $("#"+$(this).attr("id")).remove();
  });
});
function errorCampoAjaxMensaje(inp,mensaje){
  inp.before("<div class='error-fac' id='msg_error"+inp.attr("id")+"'>! "+mensaje.toUpperCase()+"</div>");
}
function isValidDate(dateString) {
 ...
 function removeFac(item,fila){
   Swal.fire({
        ...
        text: "Se borrará de la lista de facturas registradas",
         type: 'warning',
...
function createItem(text, input){
  ...
  img.src = array_auto_ajax[text.value].imagen;
  var html = img.outerHTML + " " + text.label;
  return html;
}

var autocompletarAjaxVariable=null;
 function autocompletarAjaxListado(inp,inp_val){
   var input = document.getElementById(inp);
   var input_value = document.getElementById(inp_val);
   autocompletarAjaxVariable = new Awesomplete(input, {
          item: myItemFunc,
          minChars: 1,
          maxItems: 10,
          autoFirst:true,
          ...
          replace: function(suggestion) {
              ...
              $("#tipo_empresa").val(array_auto_ajax[suggestion.value].tipo);
              $("#nombre_empresa_persona").val(array_auto_ajax[suggestion.value].nombre.toUpperCase());
              $("#pais_empresa").val(array_auto_ajax[suggestion.value].pais);
              $("#departamento_empresa").val(array_auto_ajax[suggestion.value].departamento);
              $("#ciudad_empresa").val(array_auto_ajax[suggestion.value].ciudad);
              $("#nacional_empresa").val(array_auto_ajax[suggestion.value].clase);
              $("#paterno_persona").val(array_auto_ajax[suggestion.value].paterno);
              $("#materno_persona").val(array_auto_ajax[suggestion.value].materno);
              $("#identificacion").val(array_auto_ajax[suggestion.value].identificacion);
              $("#nit_empresa").val(array_auto_ajax[suggestion.value].nit);
              $('.selectpicker').selectpicker("refresh");
              ajaxTipoProveedorPersonaSol();
              ajaxNacionalEmpresaPersonaSol();
              if(!($("#boton_guardar_nuevo_proveedor").hasClass("d-none"))){
                  $("#boton_guardar_nuevo_proveedor").addClass("d-none");
              }
              $("#datosProveedorNuevo .selectpicker").selectpicker('setStyle', 'btn-warning','remove');
              if(parseInt(array_auto_ajax[suggestion.value].proveedor)==1){
                $("#datosProveedorNuevo .selectpicker").selectpicker('setStyle', 'btn-info','add');
                $("#datosProveedorNuevo .selectpicker").selectpicker('setStyle', 'btn-danger','remove');
                if($("#boton_actualizar_lista").hasClass("d-none")){
                  $("#boton_actualizar_lista").removeClass("d-none");
                }
                if(!($("#boton_cambiar_a_proveedor").hasClass("d-none"))){
                  $("#boton_cambiar_a_proveedor").addClass("d-none");
                }
              }else{
                $("#datosProveedorNuevo .selectpicker").selectpicker('setStyle', 'btn-info','remove');
                $("#datosProveedorNuevo .selectpicker").selectpicker('setStyle', 'btn-danger','add');
                if($("#boton_cambiar_a_proveedor").hasClass("d-none")){
                  $("#boton_cambiar_a_proveedor").removeClass("d-none");
                }
                if(!($("#boton_actualizar_lista").hasClass("d-none"))){
                  $("#boton_actualizar_lista").addClass("d-none");
                }
              }
           }
          });
...
var array_auto_ajax=[];
function autocompletarAJAXComplemento(inp,url){
  array_auto_ajax=[];
 ...
 var valorDelInput = input.value;
    ...
    if(($("#boton_guardar_nuevo_proveedor").hasClass("d-none"))){
       $("#boton_guardar_nuevo_proveedor").removeClass("d-none");
      }
      if(!($("#boton_cambiar_a_proveedor").hasClass("d-none"))){
       $("#boton_cambiar_a_proveedor").addClass("d-none");
      }
      ...
      $("#tipo_empresa").val("E");
      $("#nombre_empresa_persona").val(valorDelInput.toUpperCase());
      $("#nacional_empresa").val("N");
      $("#paterno_persona").val("");
      $("#materno_persona").val("");
      $("#identificacion").val("");
      $("#nit_empresa").val("");
      ...
      $("#datosProveedorNuevo .selectpicker").selectpicker('setStyle', 'btn-danger','remove');
      $("#datosProveedorNuevo .selectpicker").selectpicker('setStyle', 'btn-info','remove');
      $("#datosProveedorNuevo .selectpicker").selectpicker('setStyle', 'btn-warning','add');
...
function agregarNuevoProveedorAjax(){
  if(autocompletarAjaxVariable.list>0){
  if($("#FormularioBusqueda").hasClass("d-none")){
    $("#FormularioBusqueda").removeClass("d-none");
    $("#FormularioProveedor").addClass("d-none");  
  }
 }else{
   if($("#FormularioProveedor").hasClass("d-none")){
    $("#FormularioProveedor").removeClass("d-none");
    $("#FormularioBusqueda").addClass("d-none");  
  }
 }
}
  ...
  if(tipoSolicitud==1||tipoSolicitud==2||tipoSolicitud==3||tipoSolicitud==4||tipoSolicitud==-1){ //-1 es para tcp tcs
   var codigoSol=$("#cod_solicitud").val();
      ...
      ajax.onreadystatechange=function(){
        if (ajax.readyState==4) {
          divDetalle.html(ajax.responseText);
          //autocompletarConImagen("partida_cuenta"+filaActiva,"partida_cuenta_id"+filaActiva,array_cuenta);
          divDetalle.bootstrapMaterialDesign();
          if($("#add_boton").length>0){
            $("#add_boton").removeAttr("disabled");
          }
...
function agregaformPCE(datos){
  ...
  var fecha_finE=d[4];
  var cod_tipocontratoE=d[5];
  var contenedor;  
  contenedor = document.getElementById('div_contenedor_fecha_fin_e');
  ajax=nuevoAjax();
  ajax.open('GET', 'personal/ajax_tipocontrato_personal.php?codigo='+cod_tipocontratoE+'&fecha_fin='+fecha_finE+'&admin=2',true);
  ajax.onreadystatechange=function() {
    if (ajax.readyState==4) {
      contenedor.innerHTML = ajax.responseText;
      $('.selectpicker').selectpicker(["refresh"]);          
...
function RegistrarContratoPersonal(cod_personal,cod_tipocontrato,fecha_inicio,fecha_fin){
  $.ajax({
    type:"POST",
    data:"cod_contrato=0&cod_personal="+cod_personal+"&cod_tipocontrato="+cod_tipocontrato+"&cod_estadoreferencial=1&fecha_inicio="+fecha_inicio+"&observaciones=''"+"&fecha_fin="+fecha_fin,
...
function EditarContratoPersonal(codigo_contratoE,codigo_personalE,cod_tipocontratoE,fecha_inicioE,fecha_fin){
  $.ajax({
    type:"POST",

    data:"cod_contrato="+codigo_contratoE+"&cod_personal=0&cod_tipocontrato="+cod_tipocontratoE+"&cod_estadoreferencial=2&fecha_inicio="+fecha_inicioE+"&observaciones=''"+"&fecha_fin="+fecha_fin,
    url:"personal/savePersonalcontrato.php",
    success:function(r){
      if(r==1){
        //$('#tabla1').load('index.php');
...
function EditarEvaluacionPersonal(codigo_contratoEv,codigo_personalEv,fecha_EvaluacionEv){
  $.ajax({
    ...
    data:"cod_contrato="+codigo_contratoEv+"&cod_personal=0&cod_tipocontrato=0&cod_estadoreferencial=4&fecha_inicio="+fecha_EvaluacionEv+"&observaciones=''"+"&fecha_fin=''",
    url:"personal/savePersonalcontrato.php",
    success:function(r){
      if(r==1){        
...
function EliminarContratoPersonal(codigo_contratoB,codigo_personalB){
  $.ajax({
    type:"POST",
    data:"cod_contrato="+codigo_contratoB+"&cod_personal=0&cod_tipocontrato=1&cod_estadoreferencial=3&fecha_inicio=0000-00-00&observaciones=''"+"&fecha_fin=''",
    url:"personal/savePersonalcontrato.php",
    success:function(r){
      if(r==1){
        //$('#tabla1').load('index.php');
...
function FinalizarContratoPersonal(codigo_contratoCF,codigo_personalCf){
  $.ajax({
    ...
    data:"cod_contrato="+codigo_contratoCF+"&cod_personal=0&cod_tipocontrato=0&cod_estadoreferencial=6&fecha_inicio='0000-00-00'&observaciones=''"+"&fecha_fin=''",
    url:"personal/savePersonalcontrato.php",
    success:function(r){
      if(r==1){   
...
function agregarEstadoCuentaCerrar(filaXXX,valor){
  ...
  for (var i = 0; i < itemEstadosCuentas.length; i++) {
    ...
    for (var j = 0; j < estadosCuenta.length; j++) {
      var estado_codigo=estadosCuenta[j].cod_comprobantedetalle;
      if(estado_codigo==detalle_resp[0]){
        montoMatado+=estadosCuenta[j].monto;
          existeEstadoCuentas++; 
          filaEstado=i+1;
      }
    };
...
function ajax_tiposContratoPersonal(combo,indice){
  var contenedor;
  var codigo_contrato=combo.value;
  if(indice==1){
    contenedor = document.getElementById('div_contenedor_fecha_fin');
  }else{
    contenedor = document.getElementById('div_contenedor_fecha_fin_e');
  }
  
  ajax=nuevoAjax();
  ajax.open('GET', 'personal/ajax_tipocontrato_personal.php?codigo='+codigo_contrato+'&fecha_fin=0&admin='+indice,true);
  ajax.onreadystatechange=function() {
    if (ajax.readyState==4) {
      contenedor.innerHTML = ajax.responseText;
      $('.selectpicker').selectpicker(["refresh"]);          
...
function guardarDatosProveedor(){
  var nombre =$("#nombre_empresa_persona").val();
  ...
  if($("#tipo_empresa").val()=='E'){
    ...
    //   nit=99001;
    // }
    // if($("#nacional_empresa").val()=="I"&&(identificacion==""||identificacion==0)){
     ...
     if(ciudad_true>0){
        ...
        if(otra==""&&ciudad_true==2){
          ...
          //proceso de guardado de informacion
           var parametros={"tipo":$("#tipo_empresa").val(),"nacional":"N","nombre":nombre,"paterno_p":"","materno_p":"","identificacion":"","nit":nit,"pais":"","estado":estado,"ciudad":"","otra":"","direccion":direccion,"telefono":telefono,"correo":correo,"nombre_contacto":nombre_contacto,"apellido_contacto":apellido_contacto,"cargo_contacto":cargo_contacto,"correo_contacto":correo_contacto};
            $.ajax({
               type: "GET",
               dataType: 'html',
               url: "ajaxAgregarNuevoProveedorSol.php",
               data: parametros,
               beforeSend: function () {
                $("#texto_ajax_titulo").html("Registrando Proveedor..."); 
                  iniciarCargaAjax();
                },
               success:  function (respuesta) {
...
function ajaxTipoProveedorPersonaSol(){
  var tipo = $("#tipo_empresa").val();
  if(tipo=="E"){
    if(!($("#div_persona_proveedor").hasClass("d-none"))){
       $("#div_persona_proveedor").addClass("d-none"); 
    }
  }else{
    if(($("#div_persona_proveedor").hasClass("d-none"))){
       $("#div_persona_proveedor").removeClass("d-none"); 
    }
  }
...
function quitarEstadoCuenta_cajachica(){
  ...
  // verEstadosCuentas_cajachica(fila,0,0,0);
  verEstadosCuentas_cajachica_filtro();
  $("#nestado"+fila).removeClass("estado");
}
function agregarEstadoCuenta_cajachica(){
  
  var controlar_seleccion = $("#controlar_seleccion").val();
  if(controlar_seleccion==0){
    $("#modalEstadosCuentas").modal("hide");
    $('#msgError').html("<p>Estado de cuenta No seleccionado.</p>");
    $("#modalAlert").modal("show");
  }else{
     iniciarCargaAjax();
    $("#mensaje_estadoscuenta").html("");
    ...
    if(tipo==1){
      ...
      if(codComproDet!=null){
        ...
        ajaxCajaCPersonalUO_cuentapasiva(codComproDet,cod_proveedorCompr,cod_estado_cuenta);//ponemos oficina  en el formulario,luego area y provee
...
function verEstadosCuentas_cajachica_filtro(){
  if($("#monto").val()=="" || $("#monto").val()==0){
   ...
   $("#modalAlert").modal("show");
    ...
    var cuenta = $("#cuenta_auto_id").val();    
    $("#cuentas_origen").val(cuenta+"###NNN");
    $('.selectpicker').selectpicker("refresh");
    $("#modalEstadosCuentas").modal("show");
  }
}
function buscarestadosCuenta_cajachica(){ 
  //var monto_cajachica=$("#monto").val();  
  var cuenta = $("#cuenta_auto_id").val();

  var modal_anio_actual = $("#modal_anio_actual").val();
  var modal_buscar_fecha = $("#modal_buscar_fecha").val();
  var modal_buscar_nombre = $("#modal_buscar_nombre").val();
  // var modal_buscar_monto = $("#modal_buscar_monto").val();
  
  var string_datos=modal_anio_actual+"#####@@@@@"+modal_buscar_fecha+"#####@@@@@"+modal_buscar_nombre;

  $("#controlar_seleccion").val("1");//indicamos que se cargaran los estados de cuenta
  iniciarCargaAjax();
  verEstadosCuentas_cajachica(1,cuenta,0,string_datos);
}

function verEstadosCuentas_cajachica(fila,cuenta,saldo_comprob,datos){    
    var monto_cajachica=$("#monto").val();
    ...
    //ajax estado de cuentas
    var parametros={"cod_cuenta":cod_cuenta,"tipo":tipo,"mes":12,"auxi":auxi,"monto_cajachica":monto_cajachica,"codigo_cajachica":codigo_cajachica,"datos":datos};
    $.ajax({
        type: "GET",
        dataType: 'html',
...
function ajaxCajaCPersonalUO_cuentapasiva(codigo_comprobante,cod_proveedor,cod_estado_cuenta){
  var contenedor;
  // var codigo_personal=combo.value;
  contenedor = document.getElementById('div_contenedor_uo');
  ajax=nuevoAjax();
  ...
  ajax.onreadystatechange=function() {
    if (ajax.readyState==4) {
      contenedor.innerHTML = ajax.responseText;
      $('.selectpicker').selectpicker(["refresh"]);
      ajaxCajaCPersonalArea_cuentapasiva(codigo_comprobante,cod_proveedor,cod_estado_cuenta);//ponemos area en el fromulario
    
    }
  }
  ajax.send(null)  
}
function ajaxCajaCPersonalArea_cuentapasiva(codigo_comprobante,cod_proveedor,cod_estado_cuenta){
  var contenedor;
  // var codigo_personal=combo.value;
  contenedor = document.getElementById('div_contenedor_area');
  ajax=nuevoAjax();
  ...
  ajax.onreadystatechange=function() {
    if (ajax.readyState==4) {
      contenedor.innerHTML = ajax.responseText;
      $('.selectpicker').selectpicker(["refresh"]);    
      ajaxCajaCProveedor_cuentapasiva(codigo_comprobante,cod_proveedor,cod_estado_cuenta);//ponemos proveedor en el fromulario  
    }
  }
  ajax.send(null)  
}
...
function ajaxCajaCProveedor_cuentapasiva(codigo_comprobante,codigo_proveedor,cod_estado_cuenta){
  ...
  ajax.onreadystatechange=function() {
    if (ajax.readyState==4) {
      ...
      $('.selectpicker').selectpicker(["refresh"]);  
      ajaxCajaCGlosa_estadocuenta(codigo_comprobante,cod_estado_cuenta);      
      // $("#modalEstadosCuentas").modal("hide");//cerramos modal  
    }
  }
  ajax.send(null)  
}
function ajaxCajaCGlosa_estadocuenta(codigo_comprobante,cod_estado_cuenta){
  var contenedor_p;
  // alert(codigo_proveedor);
...
function saveFacturaEdit(){
  var contadorError=0;  
    if(($('#nit_fac_edit').val()==''||$('#nit_fac_edit').val()==0)){
      contadorError++;
      errorCampoAjaxMensaje($('#nit_fac_edit'),'Campo requerido');
    }
    if(($('#nro_fac_edit').val()==''||$('#nro_fac_edit').val()==0)){
     contadorError++;
     errorCampoAjaxMensaje($('#nro_fac_edit'),'campo requerido');
    }
    if(($('#fecha_fac_edit').val()==''||isValidDate($('#fecha_fac_edit').val())==false)){
      contadorError++;
      errorCampoAjaxMensaje($('#fecha_fac_edit'),'Campo fecha inválido');
    }        
    if(($('#imp_fac_edit').val()==''||$('#imp_fac_edit').val()==0)){
      contadorError++;
      errorCampoAjaxMensaje($('#imp_fac'),'Monto > 0');
    }
    if($('#aut_fac_edit').val()==''){
        ...
        success:  function (resp) {
          $("#cuenta_bancaria").html(resp);
           ...
           if(!($("#nben"+fila).hasClass("estado"))){
              $("#nombre_beneficiario").val($("#proveedor"+fila+" option:selected").text().toUpperCase());
...
function ajaxNacionalEmpresaPersonaSol(){
  if($("#nacional_empresa").val()=="N"){
    if($("#nit_empresa_required").hasClass("d-none")){
      $("#nit_empresa_required").removeClass("d-none");
    }
    if($("#identificacion_required").hasClass("d-none")){
      $("#identificacion_required").removeClass("d-none");
    } 
  }else{
    if(!($("#nit_empresa_required").hasClass("d-none"))){
      $("#nit_empresa_required").addClass("d-none");
    }
    if(!($("#identificacion_required").hasClass("d-none"))){
      $("#identificacion_required").addClass("d-none");
    }
  }
}

function convertirARegistroProveedor(){
  var idCliente=$("#cod_proveedor_encontrado").val();
  if(idCliente==""||idCliente==0){
          Swal.fire("Informativo!", "Debe Seleccionar un Cliente", "warning");
  }else{
          //proceso de guardado de informacion
           var parametros={"idCliente":idCliente};
            $.ajax({
               type: "GET",
               dataType: 'html',
               url: "ajaxCambiarClienteAProveedor.php",
               data: parametros,
               beforeSend: function () {
                $("#texto_ajax_titulo").html("Asignando Atributo Proveedor..."); 
                  iniciarCargaAjax();
                },
               success:  function (respuesta) {
                  detectarCargaAjax();
...
function asignarFormaPagoGastoSR(){
  var cod_pago=$("#cod_forma_pago").val();
  if(cod_pago>0){
     if(!$("#nmoney").hasClass("estado")){
       $("#nmoney").addClass("estado");
     }
     var titulo=$("#cod_forma_pago option:selected").text().toUpperCase();
     $("#nmoney").attr("title",titulo);
  }else{
     if($("#nmoney").hasClass("estado")){
       $("#nmoney").removeClass("estado");
     }
     $("#nmoney").removeAttr("title");
  }
} 
...
function saveContaSolicitudRecursoModalCajaChica(){
  ...
  if(cod_caja==-1||cod_pago==-1){ //||cod_personal==-1
    Swal.fire("Informativo!", "Debe llenar los campos requeridos!", "warning");     
  }else{
    var parametros={"codigo":codigo,"cod_cajachica":cod_caja,"cod_pago":cod_pago}; //,"cod_personal":cod_personal
      $.ajax({
        type: "GET",
        dataType: 'html',
...
function llenarDatosTablaCuentas(){
  $("#llenado_de_cuentas").html("");
  $("#cuenta option").each(function(){
    if($(this).is(':selected')){
      var nombreCuenta=$(this).text().toUpperCase(); 
      $("#llenado_de_cuentas").append("<tr><td class='small text-left'>"+nombreCuenta.trim()+"</td><td>-</td></tr>");/*<a class='btn btn-default btn-sm btn-fab' onClick='quitarTablaCuentasMayores("+$(this)+")'><i class='material-icons'>remove_circle</i></a>*/
    }
  }); 
}
